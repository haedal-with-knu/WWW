...
- [웹 서버 운영하기(윈도우)](https://opentutorials.org/course/3084/18893)  
     ...
     #### 3. 웹서버와 HTTP
     HTTP는 Hyper Text Transfer Protocol의 약자로 인터넷에서 데이터를 주고받을때 쓰는 약속이라 보시면 될겁니다. 우리는 이 HTTP를 주소창에서 자주 볼 수 있습니다. ```Go to application```으로 접속한 웹사이트의 주소창을 보면 ```localhost```라고 되어있습니다. 이를 ```http://localhost/index.html```와 ```http://127.0.0.1/index.html```로 바꿔봅시다. 모두 같은 사이트에 접속되는것을 볼 수 있습니다.   
